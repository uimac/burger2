...
public:
	...
	SceneImpl() : is_cw_(true), pre_time_(-1) {}
	~SceneImpl() {}
	 ...
	 */
	bool load(umstring path, const UMAbcSetting& setting)
	{
	...
	bool load_material(const umstring& path)
	{
		// load material
		return umabc::UMAbcIO::import_material_map(material_map_, path);
	}

	void assign_default_material()
	{
		...
		for (int i = 0, size = static_cast<int>(name_list.size()); i < size; ++i)
		{
			...
			material_map_[material_name] = UMMaterial::default_material();
		}
	}
 ...
 * load 3d file to UMAbcScene
 */
bool UMAbcScene::load(umstring path, const UMAbcSetting& setting)
{
 ...
 * save 3d file
 */
bool UMAbcScene::save(umstring path, UMAbcScenePtr scene, const UMAbcSetting& setting)
{
