...
void UMAbcMesh::update_normal()
{
	...
	if (opengl_mesh_ && !umdraw::UMDirectX11::current_device_pointer())
	{
		...
		{
			opengl_mesh_->update_normal(triangle_index_, original_normal_, UMOpenGLAbcMesh::eFaceVarying);
		}
		else
		{
			opengl_mesh_->update_normal(triangle_index_, original_normal_, UMOpenGLAbcMesh::eVertexVarying);
		}
	}
#endif // WITH_OPENGL
...
void UMAbcMesh::update_uv()
{
	...
	if (opengl_mesh_ && !umdraw::UMDirectX11::current_device_pointer())
	{
		if (scope == kFacevaryingScope)
		{
			opengl_mesh_->update_uv(triangle_index_, uv_, UMOpenGLAbcMesh::eFaceVarying);
		}
		else
		{
			opengl_mesh_->update_uv(triangle_index_, uv_, UMOpenGLAbcMesh::eVertexVarying);
		}
	}
#endif // WITH_OPENGL
...
void UMAbcMesh::update_vertex(IPolyMeshSchema::Sample& sample)
{
	...
	if (opengl_mesh_ && !umdraw::UMDirectX11::current_device_pointer())
	{
		opengl_mesh_->update_vertex(triangle_index_, vertex_);
	}
#endif // WITH_OPENGL
